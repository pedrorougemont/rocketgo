...
const markMessasgeAsAnswered = `-- name: MarkMessasgeAsAnswered :exec
UPDATE messages
SET
    answered = true
WHERE
...
func (q *Queries) MarkMessasgeAsAnswered(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, markMessasgeAsAnswered, id)
	return err
}
